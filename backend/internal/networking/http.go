...
func (s *Server) StartHTTP(addr string) {
	...
	mux.HandleFunc("/api/keys/value",      s.cors(s.handleKeyValue))
...
func (s *Server) handleKeys(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodDelete:
		...
		if key == "" { http.Error(w, "missing ?key= param", 400); return }
		n := s.Store.Del(key)
...
// handleKeyValue fetches the actual value of a single key for the Edit modal.
func (s *Server) handleKeyValue(w http.ResponseWriter, r *http.Request) {
	...
	t := s.Store.Type(key)
	if t == "" {
		writeJSON(w, map[string]any{"error": "key not found"})
		return
	}
	...
	case "String":
		...
		value = v
	case "List":
		items := s.Store.LRange(key, 0, -1)
		b, _ := json.Marshal(items)
		value = string(b)
	case "Set":
		items := s.Store.SMembers(key)
		b, _ := json.Marshal(items)
		value = string(b)
	case "ZSet":
		items := s.Store.ZRange(key, 0, -1)
		b, _ := json.Marshal(items)
		value = string(b)
	}
...
func (s *Server) handleExec(w http.ResponseWriter, r *http.Request) {
	...
	if body.Cmd == "" { http.Error(w, "empty cmd", 400); return }
